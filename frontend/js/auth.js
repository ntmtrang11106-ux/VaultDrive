    ...
    if (registerForm) {
        registerForm.addEventListener('submit', async (e) => {
            ...
            if (!phoneRegex.test(phone)) {
                showFieldError('phone', 'Số điện thoại không hợp lệ (phải đúng 10 chữ số).');
                hasError = true;
            }
            ...
            if (!pwdRegex.test(password)) {
                showFieldError('password', 'Mật khẩu phải từ 8 ký tự, bắt đầu bằng chữ hoa, có số và ký tự đặc biệt.');
                hasError = true;
            }
            ...
            try {
                ...
                const response = await fetch(`${API_BASE_URL}/register`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ fullname, email, phone, dob, gender, password })
                });

                if (!response.ok) {
                    const text = await response.text();
                    throw new Error(text || 'Đăng ký thất bại. Email có thể đã được sử dụng.');
                }

                // Nếu đăng ký thành công, báo xanh và đếm ngược chuyển sang trang Đăng nhập
                successEl.textContent = 'Đăng ký thành công! Đang chuyển hướng đến trang đăng nhập...';
    ...
    if (forgotForm) {
        forgotForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                successEl.textContent = 'Liên kết khôi phục đã được gửi đến email của bạn.';
                successEl.style.display = 'block';
            } catch (err) {
